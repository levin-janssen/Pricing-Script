...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_settings']) && empty($db_error)) {
    $posted_asin = filter_input(INPUT_POST, 'asin', FILTER_SANITIZE_STRING); // ASIN is the key for Preisgrenzen
    ...
    $selectedAsin = $posted_asin ?: filter_input(INPUT_GET, 'asin', FILTER_SANITIZE_STRING);
    ...
    elseif ($min_preis === false || $min_preis < 0) { $update_message = 'Fehler: Ungültiger Minimalpreis.'; $validation_passed = false; }
    elseif ($max_preis === false || $max_preis < 0) { $update_message = 'Fehler: Ungültiger Maximalpreis.'; $validation_passed = false; }
    elseif ($min_preis > $max_preis) { $update_message = 'Fehler: Minimalpreis > Maximalpreis.'; $validation_passed = false; }
    elseif ($stepsize_small === false || $stepsize_small <= 0) { $update_message = 'Fehler: Kleine Schrittgröße <= 0.'; $validation_passed = false; }
    elseif ($stepsize_big === false || $stepsize_big <= 0) { $update_message = 'Fehler: Große Schrittgröße <= 0.'; $validation_passed = false; }
    elseif ($stepsize_small > $stepsize_big) { $update_message = 'Fehler: Kleine Schrittgröße > Große Schrittgröße.'; $validation_passed = false; }
    ...
    if (isset($_GET['asin']) && !empty($_GET['asin'])) {
        $selectedAsin = filter_input(INPUT_GET, 'asin', FILTER_SANITIZE_STRING);
    ...
    try {
        ...
        if (!$productArtikelDetails) {
            ...
            if ($update_message_type !== 'error') {
             ...
             if (empty($form_min_preis)) $form_step_small = '0.01'; // Ensure defaults if db returns null
...
<body>
    ...
    align-items: center;
    gap: .25em;
"> <img src="../img/<?= htmlspecialchars($current_marketplace_code) ?>.png" alt="<?= htmlspecialchars($current_marketplace_code) ?>" style="height: 1em; vertical-align: middle;"><a target="_blank" href="https://www.amazon.de/dp/<?= htmlspecialchars($selectedAsin ?: 'N/A') ?>"><?= htmlspecialchars($selectedAsin ?: 'Keine ASIN') ?></a></h1>
        </div>
    ...
    <?php elseif ($productArtikelDetails): // We need at least Artikel details to show something ?>
        ...
        <div class="product-info">
            <h3><?= htmlspecialchars($productArtikelDetails['artikelname']) ?> (SKU: <?= htmlspecialchars($productArtikelDetails['sku'] ?? 'N/A') ?>)</h3>
            <div class="price-fields">
                <div class="price-field" title="Eigener Preis"><img src="../img/person.png" alt="Eigener Preis" class="price-icon"><span class="price-value"><?= ($productArtikelDetails['eigener_preis'] !== 'N/A' && $productArtikelDetails['eigener_preis'] !== null) ? number_format((float)$productArtikelDetails['eigener_preis'], 2, '.', '').$currency_symbol : 'N/A' ?></span></div>
                <div class="price-field" title="Niedrigster Preis"><img src="../img/arrow_down.png" alt="Niedrigster Preis" class="price-icon"><span class="price-value"><?= ($productArtikelDetails['niedrigster_preis'] !== 'N/A' && $productArtikelDetails['niedrigster_preis'] !== null) ? number_format((float)$productArtikelDetails['niedrigster_preis'], 2, '.', '').$currency_symbol : 'N/A' ?></span></div>
                <div class="price-field" title="BuyBox Preis"><img src="../img/star.png" alt="BuyBox Preis" class="price-icon"><span class="price-value"><?= ($productArtikelDetails['bbox_preis'] !== 'N/A' && $productArtikelDetails['bbox_preis'] !== null) ? number_format((float)$productArtikelDetails['bbox_preis'], 2, '.', '').$currency_symbol : 'N/A' ?></span></div>
        ...
        <div class="price-history-graph">
            ...
            <div class="chart-and-settings-wrapper">
                <div class="chart-container">
                    ...
                    <?php else: ?>
                        <p>Keine Preisdaten für Diagramm in Land <?= htmlspecialchars($current_marketplace_code) ?> verfügbar.</p>
                    <?php endif; ?>
                </div>

                <form class="compact-settings-form" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>?asin=<?= urlencode($selectedAsin) ?>" method="POST">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" id="min_preis" name="min_preis" required value="<?= htmlspecialchars($form_min_preis) ?>" title="Minimalpreis">
                    </div>
                    <div class="input-group">
                        <label for="max_preis">Max Preis (<?= $currency_symbol ?>):</label>
                        <input type="number" step="0.01" id="max_preis" name="max_preis" required value="<?= htmlspecialchars($form_max_preis) ?>" title="Maximalpreis">
                    </div>
                    <div class="input-group">
                        <label for="stepsize_small">Step Klein (<?= $currency_symbol ?>):</label>
                        <input type="number" step="0.01" id="stepsize_small" name="stepsize_small" required value="<?= htmlspecialchars($form_step_small) ?>" title="Kleine Schrittgröße (<10 <?= $currency_symbol ?>)">
                    </div>
                    <div class="input-group">
                        <label for="stepsize_big">Step Groß (<?= $currency_symbol ?>):</label>
                        <input type="number" step="0.01" id="stepsize_big" name="stepsize_big" required value="<?= htmlspecialchars($form_step_big) ?>" title="Große Schrittgröße (>=10 <?= $currency_symbol ?>)">
                    </div>
        ...
        let currentChart;
        // Min/Max Preis für die Linien im Chart kommen jetzt von productPreisgrenzen
        ...
        function createChart(dataToDisplay) {
            ...
            if (dataToDisplay && dataToDisplay.length > 1) {
                ...
                for (let i = 1; i < dataToDisplay.length; i++) {
                    console.log(dataToDisplay[i].isWinner);
            ...
            console.log("Buybox Ja: " + buyboxyes + ", Buybox Nein: " + buyboxno);
    ...
    <?php elseif (!empty($_GET['asin']) && !$db_error): ?>
        <p class="message error">Keine Produktdetails für die ASIN "<?= htmlspecialchars($selectedAsin) ?>" in Land <?= htmlspecialchars($current_marketplace_code) ?> gefunden oder konfiguriert.</p>
    <?php endif; ?>
