...
try {
    // 1. FBA Bestand abrufen
    $stmtFba = $dbConnectionTric->query("
        SELECT vk_ID as produktid, SUM(menge) as fba_stock 
        FROM lager 
        WHERE lagerplatz IN ($fba_places) 
        GROUP BY vk_ID
    ");
    if (!$stmtFba) {
        throw new PDOException("SQL Fehler in FBA-Abfrage: " . $dbConnectionTric->errorInfo()[2]);
    }
    $fbaStocks = [];
    while ($row = $stmtFba->fetch(PDO::FETCH_ASSOC)) {
        $fbaStocks[$row['produktid']] = (int)$row['fba_stock'];
    }

    // 2. Lokalen Bestand abrufen (Alles außer FBA)
    $stmtLocal = $dbConnectionTric->query("
        SELECT vk_ID as produktid, SUM(menge) as local_stock 
        FROM lager 
        WHERE lagerplatz NOT IN ($fba_places) 
        GROUP BY vk_ID
    ");
    if (!$stmtLocal) {
        throw new PDOException("SQL Fehler in Lokal-Abfrage: " . $dbConnectionTric->errorInfo()[2]);
    }
    $localStocks = [];
    ...
    // 4. Tricoma Produktdaten (Titel, SKU, ASIN) in EINER Abfrage bündeln
    ...
    ");
    if (!$stmtFields) {
        throw new PDOException("SQL Fehler in Produktdaten-Abfrage: " . $dbConnectionTric->errorInfo()[2]);
    }
    ...
    while ($row = $stmtFields->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($val !== '') {
            ...
            if ($fid === 44) $tricomaData[$pid]['sku'] = $val;   // Artikelnummer
            if ($fid === 57) $tricomaData[$pid]['asin'] = $val;  // ASIN
        }
    }

    // 5. Fallback aus tric4calc (mit Case-Insensitive SKU Mapping)
    $stmtArtikel = $dbConnectionTric4Calc->query("SELECT sku, asin, artikelname FROM Artikel");
    $calcArticles = [];
    if ($stmtArtikel) {
